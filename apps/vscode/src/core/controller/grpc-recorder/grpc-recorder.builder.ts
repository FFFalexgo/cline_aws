	...
	static getRecorder(controller: Controller): IRecorder {
		if (!GrpcRecorderBuilder.recorder) {
			GrpcRecorderBuilder.recorder = GrpcRecorder.builder()
				...
				// These RPCs carry AWS secrets and must never enter recorder logs.
				.withFilters((request) => ["updateBedrockCredentials", "revealBedrockCredentials"].includes(request.method))
				.build(controller)
		}
...
function testFilters(): GrpcRequestFilter[] {
	...
	return [
		...
		(req) => ["bedrock_coder.UiService", "bedrock_coder.McpService", "bedrock_coder.WebService"].includes(req.service),
		(req) =>
