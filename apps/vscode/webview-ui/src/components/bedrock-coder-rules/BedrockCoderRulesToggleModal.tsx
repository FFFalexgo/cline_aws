...
const BedrockCoderRulesToggleModal: React.FC = () => {
	const {
		...
		hooksEnabled,
		setGlobalBedrockCoderRulesToggles,
		setLocalBedrockCoderRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalBedrockCoderRulesToggles?.toggles) {
						setGlobalBedrockCoderRulesToggles(response.globalBedrockCoderRulesToggles.toggles)
					}
					if (response.localBedrockCoderRulesToggles?.toggles) {
						setLocalBedrockCoderRulesToggles(response.localBedrockCoderRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalBedrockCoderRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localBedrockCoderRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleBedrockCoderRule(
			ToggleBedrockCoderRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling Bedrock Coder rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: BedrockCoderRulesToggles) => {
				if (response.toggles) {
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Bedrock Coder with system-level guidance. Think of them as a
								...
								<p>
									Skills are reusable instruction sets that Bedrock Coder can activate on-demand. When a task
									matches a skill's description, Bedrock Coder uses the{" "}
									<span className="font-bold">use_skill</span> tool to load the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Bedrock Coder's execution
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.bedrock-coder/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
...
export default BedrockCoderRulesToggleModal
